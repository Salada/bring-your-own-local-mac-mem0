...
Neural reranking is separately opt-in. Install the optional dependencies with
`uv sync --extra rerank`, then check device availability:
...
embedding migration or backfill. Mem0 retains the
vector `score` and adds `rerank_score`; temporal queries still apply their
bounded date boost. See [ADR 0004](../docs/decisions/0004-opt-in-reranker.md).
