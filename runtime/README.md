...
Updates require the exact current `expected_hash`, `expected_revision`, and
`expected_user_id` (plus `expected_app_id` when applicable) from a prior ID
lookup; stale or cross-scope updates are rejected. Category backfill also scans
expired records, but its apply mode remains explicitly operator-controlled.
